...
Describe 'Assert-BitwiseFlag' {
    ...
    Context 'When asserting flag is set on single values' {
        It 'Should pass when flag is set' {
            $null = Assert-BitwiseFlag -Actual 7 -Flag 4
        }

        It 'Should pass when multiple flags are set' {
            $null = Assert-BitwiseFlag -Actual 15 -Flag 5
        }

        It 'Should throw when flag is not set' {
            { Assert-BitwiseFlag -Actual 3 -Flag 4 } | Should -Throw -ExpectedMessage "*flag '4' set*not set on '3'*"
        }

        It 'Should pass when flag is set with zero flag value' {
            $null = Assert-BitwiseFlag -Actual 7 -Flag 0
        }
        ...
        It 'Should pass when using enum flag that is set' {
            ...
            $null = Assert-BitwiseFlag -Actual $fileAttributes -Flag ([System.IO.FileAttributes]::ReadOnly)
        }

        It 'Should throw when using enum flag that is not set' {
            $fileAttributes = [System.IO.FileAttributes]::ReadOnly
            { Assert-BitwiseFlag -Actual $fileAttributes -Flag ([System.IO.FileAttributes]::Hidden) } | Should -Throw
        }

        It 'Should pass with large Int64 values' {
            $value = [System.Int64]0x8000000000000001
            $flag = [System.Int64]0x8000000000000000
            $null = Assert-BitwiseFlag -Actual $value -Flag $flag
        }

        It 'Should pass with negative values' {
            $null = Assert-BitwiseFlag -Actual (-1) -Flag 1
        }
    }
    ...
    Context 'When asserting flag on array with -Each parameter' {
        ...
        It 'Should throw with empty array' {
            $values = @()
            { Assert-BitwiseFlag -Actual $values -Flag 4 -Each } | Should -Throw -ExpectedMessage "*bitwise-compatible type*"
        }
        ...
        It 'Should throw when array contains null element' {
            $values = @(7, $null, 15)
            { Assert-BitwiseFlag -Actual $values -Flag 4 -Each } | Should -Throw -ExpectedMessage "*not to be null*"
        }
    ...
    Context 'When asserting flag on array without -Each parameter' {
        It 'Should throw when checking array object itself' {
            $values = @(7, 15, 23)
            { Assert-BitwiseFlag -Actual $values -Flag 4 } | Should -Throw -ExpectedMessage "*bitwise-compatible type*"
        }
    }

    Context 'When using pipeline input' {
        It 'Should pass when piping single value with flag set' {
            $null = 7 | Assert-BitwiseFlag -Flag 4
        }
        ...
        It 'Should not unwrap single-element array with -Each' {
            $null = @(7) | Assert-BitwiseFlag -Flag 4 -Each
        }
    }
    ...
    Context 'When validating input types' {
        ...
        It 'Should throw when actual value is string' {
            { Assert-BitwiseFlag -Actual 'test' -Flag 4 } | Should -Throw -ExpectedMessage "*bitwise-compatible type*"
        }

        It 'Should throw when actual value is decimal' {
            { Assert-BitwiseFlag -Actual 123.45 -Flag 4 } | Should -Throw -ExpectedMessage "*bitwise-compatible type*"
        }

        It 'Should throw when actual value is boolean' {
            { Assert-BitwiseFlag -Actual $true -Flag 4 } | Should -Throw -ExpectedMessage "*bitwise-compatible type*"
        }

        It 'Should throw when actual value is hashtable' {
            { Assert-BitwiseFlag -Actual @{ Key = 'Value' } -Flag 4 } | Should -Throw -ExpectedMessage "*bitwise-compatible type*"
        }

        It 'Should pass when flag is enum and actual is integer' {
            $null = Assert-BitwiseFlag -Actual 1 -Flag ([System.IO.FileAttributes]::ReadOnly)
        }

        It 'Should pass when flag is integer and actual is enum' {
            $fileAttributes = [System.IO.FileAttributes]::ReadOnly
            $null = Assert-BitwiseFlag -Actual $fileAttributes -Flag 1
        }
    }
    ...
    Context 'When using -Because parameter' {
        It 'Should include reason in error message' {
            ...
            { Assert-BitwiseFlag -Actual 3 -Flag 4 -Because $reason } | Should -Throw -ExpectedMessage "*$reason*"
        }

        It 'Should include reason in error message with -Each' {
            $values = @(7, 3, 15)
            $reason = 'all values must have flag'
            { Assert-BitwiseFlag -Actual $values -Flag 4 -Each -Because $reason } | Should -Throw -ExpectedMessage "*$reason*"
        }
    }

    Context 'When using Should-HaveFlag alias' {
        It 'Should work with alias' {
            $null = Should-HaveFlag -Actual 7 -Flag 4
        }

        It 'Should throw with alias when flag not set' {
            { Should-HaveFlag -Actual 3 -Flag 4 } | Should -Throw
        }
    }
    ...
    Context 'When testing various integer types' {
        It 'Should pass with Byte type' {
            $value = [System.Byte]7
            $null = Assert-BitwiseFlag -Actual $value -Flag 4
        }
    ...
    Context 'When testing edge cases with bitwise operations' {
        It 'Should pass when all bits are set' {
            $null = Assert-BitwiseFlag -Actual 0xFF -Flag 0x80
        }
        ...
        It 'Should throw when only some of the flag bits are set' {
            $value = 0b11010000
            $flag = 0b11110000
            { Assert-BitwiseFlag -Actual $value -Flag $flag } | Should -Throw
        }

        It 'Should pass with zero value and zero flag' {
            $null = Assert-BitwiseFlag -Actual 0 -Flag 0
        }

        It 'Should throw with zero value and non-zero flag' {
            { Assert-BitwiseFlag -Actual 0 -Flag 4 } | Should -Throw
        }
    }
}
